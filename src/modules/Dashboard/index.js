...
const Dashboard = () => {
	...
	useEffect(() => {
		setSocket(io("https://chatappbackend-ur02.onrender.com"))
	}, [])
	...
	useEffect(() => {
		...
		const fetchConversations = async () => {
			const res = await fetch(`https://chatappbackend-ur02.onrender.com/api/conversations/${loggedInUser?.id}`, {
				method: 'GET',
				headers: {
	...
	useEffect(() => {
		const fetchUsers = async () => {
			const res = await fetch(`https://chatappbackend-ur02.onrender.com/api/users/${user?.id}`, {
				method: 'GET',
				headers: {
					'Content-Type': 'application/json',
				}
	...
	const fetchMessages = async (conversationId, receiver) => {
		const res = await fetch(`https://chatappbackend-ur02.onrender.com/api/message/${conversationId}?senderId=${user?.id}&&receiverId=${receiver?.receiverId}`, {
			method: 'GET',
			headers: {
				'Content-Type': 'application/json',
			}
	...
	const sendMessage = async (e) => {
		...
		const res = await fetch(`https://chatappbackend-ur02.onrender.com/api/message`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
		...
		<div className='w-screen flex'>
			<div className='w-[25%] h-screen bg-secondary overflow-scroll'>
				<div className='flex items-center my-8 mx-14'>
					<div><img src={img7} width={75} height={75} className='border border-primary p-[2px] rounded-full' /></div>
					<div className='ml-8'>
